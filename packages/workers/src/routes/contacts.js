    ...
    try {
        ...
        await db
          ...
          company, job_title, status, created_by_id, created_at, updated_at
            ...
            .bind(contactId, user.tenantId, first_name, last_name, email || null, phone || null, company || null, job_title || null, 'lead', user.userId, now, now)
            .run();
...
contactRouter.put('/:id', async (c) => {
    try {
        ...
        await db
         ...
         WHERE id = ?`)
            .bind(updates.first_name, updates.last_name, updates.email, updates.phone, updates.company, updates.job_title, updates.status, updates.notes, now, contactId)
            .run();
